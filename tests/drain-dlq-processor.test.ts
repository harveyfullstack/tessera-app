...
function createProcessor(budget = 8) {
  ...
  return { processor, jobRecords, attemptRpc };
}

async function failTimes(jobRecords: JobRecordService, jobId: string, times: number) {
  for (let i = 0; i < times; i += 1) {
    await jobRecords.markRunning(jobId, `worker-${i}`);
    await jobRecords.markFailed(jobId, `failure ${i}`, { errorBody: `body-${i}` });
    await jobRecords.retry(jobId);
  }
}
...
describe("DrainDlqProcessor", () => {
  test("marks jobs past the retry budget as failed during drain", async () => {
    ...
    await failTimes(jobRecords, job.id, 3);
  ...
  test("builds dlq summaries with last three attempt bodies per endpoint", async () => {
    ...
    const summaries = await processor.buildDlqSummaries(baseInput.briefId, "acct-1");
    expect(summaries).toHaveLength(1);
    expect(summaries[0]?.pastBudgetCount).toBe(1);
    expect(summaries[0]?.lastAttempts.length).toBeGreaterThan(0);
    expect(summaries[0]?.lastAttempts[0]?.errorBody).toBe("body-2");
  });

  test("supports listing jobs filtered to DLQ status", async () => {
    const { jobRecords } = createProcessor(2);
    const job = await jobRecords.ensureJobRecord(baseInput);
    await failTimes(jobRecords, job.id, 2);

    const rows = await jobRecords.listByBrief(baseInput.briefId);
    const dlqJobs = [];
