...
BEGIN
  ...
  -- Aggregate parity: every surviving delivery_job's backfilled row count
  -- matches the legacy CASE used by 002, and the grand totals match.
  SELECT COALESCE(SUM(
    CASE
      WHEN status = 'queued' AND retry_count = 0 THEN 0
      WHEN status IN ('queued', 'completed', 'running', 'cancelled') THEN retry_count + 1
      ELSE GREATEST(retry_count, 1)
    END
  ), 0) INTO old_total FROM delivery_jobs;

  SELECT COUNT(*) INTO new_total FROM delivery_attempts;

  IF old_total IS DISTINCT FROM new_total THEN
    RAISE EXCEPTION 'aggregate attempt-count parity failed: old=% new=%', old_total, new_total;
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE counts.attempt_count IS DISTINCT FROM (
      CASE
        WHEN dj.status = 'queued' AND dj.retry_count = 0 THEN 0
        WHEN dj.status IN ('queued', 'completed', 'running', 'cancelled') THEN dj.retry_count + 1
        ELSE GREATEST(dj.retry_count, 1)
      END
    )
  ) THEN
    RAISE EXCEPTION 'per-job attempt-count parity failed';
  END IF;
END $$;
