-- Isolated fixture for validating 002 duplicate-intent reconciliation and
-- old/new attempt-count parity. Apply against a throwaway database.
...
INSERT INTO jobs (
  ...
  -- running with 2 prior failures: must backfill 2 failed + 1 running.
  ...
  -- cancelled with 1 prior failure: must backfill 1 failed + 1 cancelled.
  ...
  -- requeued with 2 prior failures: must backfill 2 failed + 1 queued.
...
BEGIN
  ...
  -- Aggregate parity: every remaining delivery job's diagnostic (failed +
  -- completed) attempt rows must match the legacy retry-derived count.
  IF (
    SELECT SUM(
      CASE
        WHEN status = 'completed' THEN retry_count + 1
        WHEN status = 'failed' THEN GREATEST(retry_count, 1)
        ELSE retry_count
      END
    )
    FROM delivery_jobs
  ) <> (
    SELECT COUNT(*) FROM delivery_attempts WHERE status IN ('failed', 'completed')
  ) THEN
    RAISE EXCEPTION 'aggregate old/new diagnostic attempt-count parity failed';
  END IF;
END $$;
