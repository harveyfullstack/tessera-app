...
-- Attempt-count parity is asserted per job and in aggregate before cutover.
...
BEGIN
  ...
  CROSS JOIN LATERAL (
    SELECT CASE
      WHEN dj.status = 'queued' AND dj.retry_count = 0 THEN 0
      WHEN dj.status = 'queued' THEN dj.retry_count + 1
      WHEN dj.status IN ('completed', 'running', 'cancelled') THEN dj.retry_count + 1
      ELSE GREATEST(dj.retry_count, 1)
    END AS expected
  ) exp
  WHERE exp.expected <> (
    SELECT COUNT(*) FROM delivery_attempts da WHERE da.delivery_job_id = dj.id
  );

  IF mismatched <> 0 THEN
    RAISE EXCEPTION 'per-job attempt-count parity failed for % delivery jobs', mismatched;
  END IF;

  SELECT COALESCE(SUM(
    CASE
      WHEN status = 'queued' AND retry_count = 0 THEN 0
      WHEN status = 'queued' THEN retry_count + 1
      WHEN status IN ('completed', 'running', 'cancelled') THEN retry_count + 1
